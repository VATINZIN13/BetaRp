...
const updateDrag = () => {
	...
	$(".populated").tooltip({
		create: function(event,ui){
			...
			if (name == "Passaporte"){
				$(this).tooltip({
					content: `<item>${name}</item><br><description>Informações são mostradas após utilizar.</description><br><legenda>Tipo: <r>Consumível</r> <s>|</s> Peso: <r>0.1</r></legenda>`,
				...
				var typeName = "Serial"
				if (name == "Ferramentas Avançadas"){
					serial = parseInt(serial)
					typeName = "Cargas"
				}

				$(this).tooltip({
					content: `<item>${name}</item>${desc !== "undefined" ? "<br><description>"+desc+"</description>":""}<br><legenda>${serial === "undefined" ? "Tipo: <r>"+type+"</r>": typeName+": <r>"+serial+"</r>"} <s>|</s> Máximo: <r>${max !== "undefined" ? max:"S/L"}</r></legenda>`,
					position: { my: "center top+10", at: "center bottom", collision: "flipfit" },
					show: { duration: 10 },
					hide: { duration: 10 }
				});
