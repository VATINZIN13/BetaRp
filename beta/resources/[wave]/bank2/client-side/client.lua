...
vSERVER = Tunnel.getInterface("bank2")
-----------------------------------------------------------------------------------------------------------------------------------------
-- LOCALIDADES
-----------------------------------------------------------------------------------------------------------------------------------------
local localidades = {
...
Citizen.CreateThread(function()
	...
	for k,v in pairs(localidades) do
		table.insert(innerTable,{ v[1],v[2],v[3],2,"E","Wave Bank","Pressione para abrir" })
	end
...
RegisterNUICallback("requestBank",function(data,cb)
	local resultado,nome,sobrenome,horas,analise,recusas = vSERVER.requestBank()
	if resultado then
		cb({ resultado = resultado, nome = nome, sobrenome = sobrenome, horas = horas, analise = analise, recusas = recusas })
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- REQUESTFINES
