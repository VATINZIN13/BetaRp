...
function updateDisplayHud()
	...
	if IsPedInAnyVehicle(ped) then
		...
		SendNUIMessage({ vehicle = true, talking = talking, health = health, armour = armour, thirst = clientThirst, hunger = clientHunger, stress = clientStress, street = streetName, direction = flexDirection, oxigen = clientOxigen, suit = divingMask, fuel = fuel, speed = speed, showbelt = showBelt, seatbelt = beltLock, hardness = (hardness[plate] or 0) })
	else
		if IsMinimapRendering() then
			DisplayRadar(false)
		end

		SendNUIMessage({ vehicle = false, talking = talking, health = health, armour = armour, thirst = clientThirst, hunger = clientHunger, stress = clientStress, street = streetName, direction = flexDirection, oxigen = clientOxigen, suit = divingMask })
	end
end
-----------------------------------------------------------------------------------------------------------------------------------------
-- HUD
			...
			if IsPedInAnyVehicle(ped) then
				if not IsPedOnAnyBike(ped) and not IsPedInAnyHeli(ped) and not IsPedInAnyPlane(ped) then
					...
					if speed ~= beltSpeed then
						...
						if ((beltSpeed - speed) >= 40 and beltLock == 0) then
